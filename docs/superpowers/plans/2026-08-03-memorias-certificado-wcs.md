...
/* La tarjeta enmarca tanto el video embebido como el enlace de respaldo.
   `position: relative` es obligatorio: el script de Instagram inserta su
   iframe con `position: absolute`, y sin un ancestro posicionado se
   anclaría al documento y aparecería en cualquier parte de la página. */
.reel {
  position: relative;
  display: flex;
  align-items: center;
...
.reel .instagram-media-registered .boton { display: none; }
